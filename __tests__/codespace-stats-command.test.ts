...
// Mock the codespaces module functions
vi.mock('../src/codespaces.js', () => ({
  runCodespaceStats: vi.fn(),
}));

describe('Commands - codespace-stats-command', () => {
  describe('codespace-stats-command', () => {
    it('should be defined with correct name and description', () => {
      expect(codespaceStatsCommand.name()).toBe('codespace-stats');
      expect(codespaceStatsCommand.description()).toContain(
        'codespace usage statistics',
      );
    });

    it('should have required options defined', () => {
      const options = codespaceStatsCommand.options;
    ...
    it('should have default values for certain options', () => {
      const baseUrlOption = codespaceStatsCommand.options.find(
        (opt) => opt.long === '--base-url',
      );
      expect(baseUrlOption?.defaultValue).toBe('https://api.github.com');

      const pageSizeOption = codespaceStatsCommand.options.find(
        (opt) => opt.long === '--page-size',
      );
      expect(pageSizeOption?.defaultValue).toBe(100);

      const outputDirOption = codespaceStatsCommand.options.find(
    ...
    it('should have numeric defaults as numbers not strings', () => {
      ...
      numericOptions.forEach(({ name, expected }) => {
        const option = codespaceStatsCommand.options.find(
          (opt) => opt.long === name,
        );
    ...
    it('should have multi-org options properly configured', () => {
      const orgListOption = codespaceStatsCommand.options.find(
      ...
      const delayOption = codespaceStatsCommand.options.find(
      ...
      const continueOnErrorOption = codespaceStatsCommand.options.find(
