...
export async function initCommand(
  ...
  if (orgsToProcess.length === 1 && orgName) {
    ...
    stateManager = new StateManager(outputDir, orgName, logger, config.statePrefix);
...
async function executeForOrg(
  ...
  try {
    ...
    const outputDir = opts.outputDir || 'output';
    const stateManager = new StateManager(outputDir, orgName, logger, config.statePrefix);
